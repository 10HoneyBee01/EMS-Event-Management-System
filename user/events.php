...
$user_id = $_SESSION['user_id']; // Ensure the user is logged in before fetching their events
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['delete_event_id'])) {
    ...
    $stmt->bind_param("ii", $event_id, $user_id); // Ensure the event belongs to the user
...
if ($_SERVER['REQUEST_METHOD'] == 'GET' && isset($_GET['download_event_id'])) {
    ...
    // Fetch event name
    $event_stmt = $conn->prepare("SELECT name FROM events WHERE id = ? AND created_by = ?");
...
if ($search_term) {
    $stmt = $conn->prepare("SELECT * FROM events WHERE (name LIKE ? OR description LIKE ?) AND created_by = ? LIMIT ? OFFSET ?");
    $stmt->bind_param("ssiii", $search_term, $search_term, $user_id, $limit, $offset);
} else {
    $stmt = $conn->prepare("SELECT * FROM events WHERE created_by = ? LIMIT ? OFFSET ?");
...
$total_stmt = $conn->prepare("SELECT COUNT(*) FROM events WHERE created_by = ?");
    ...
    <div class='d-flex align-items-center'>
        <div class='progress' style='width: 80%; margin-right: 10px;'>
            <div class='progress-bar $progress_color' role='progressbar' style='width: {$progress_percentage}%;' aria-valuenow='{$progress_percentage}' aria-valuemin='0' aria-valuemax='100'>
                {$attendee_count} / {$max_capacity} <!-- Showing the attendee count and max capacity -->
            </div>
        </div>
        <span>{$progress_percentage}%</span> <!-- Display the percentage next to the progress bar -->
    </div>
</td>
