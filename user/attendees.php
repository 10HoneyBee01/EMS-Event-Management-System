<?php
ob_start();
require '../database/db.php'; // Adjusted to remove '../' if this script is in the admin folder
...
if (isset($_GET['delete_attendee'])) {
    ...
    // Prepare SQL to delete attendee
...
if (isset($_POST['add_attendee'])) {
    ...
    // Redirect after adding attendee
    header("Location: attendees.php");
    exit();
}
...
if (isset($_POST['update_attendee'])) {
    ...
    // Redirect after updating attendee info
    header("Location: attendees.php");
    exit();
}
...
$events_query = $conn->prepare("SELECT * FROM events WHERE created_by = ?");
...
// Loop through each event to handle attendees and pagination
foreach ($events as $event) {
    $event_id = $event['id'];
    $limit = 5; // Number of attendees per page for each event
    $page_param = "page_$event_id"; // Unique page parameter for this event
    ...
    if ($search_term) {
        $count_stmt = $conn->prepare("SELECT COUNT(*) FROM attendees WHERE (name LIKE ? OR email LIKE ?) AND event_id = ?");
        $count_stmt->bind_param("ssi", $search_term, $search_term, $event_id);
    } else {
